...
def save_depth():
    ...
    test_dataset = MVSDataset(args.testpath, args.testlist, "test", 5, args.numdepth)
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
    depth_reprojected, x2d_reprojected, y2d_reprojected = reproject_with_depth(depth_ref, intrinsics_ref, extrinsics_ref,
...
def check_photometric_confidence(confidence):
...
def filter_depth(scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_img = read_img(os.path.join(scan_folder, 'images/rect_{:0>3}_{}_r5000.png'.format(ref_view+1,3)))
        # load the estimated depth of the reference view
        ref_depth_est = read_pfm(os.path.join(out_folder, 'depth_est/{:0>8}.pfm'.format(ref_view)))[0]
        # load the photometric mask of the reference view
        confidence = read_pfm(os.path.join(out_folder, 'confidence/{:0>8}.pfm'.format(ref_view)))[0]
        photo_mask = check_photometric_confidence(confidence)
        ...
        color = ref_img[1::4, 1::4, :][valid_points]  # hardcoded for DTU dataset
...
if __name__ == '__main__':
    ...
    for scan in scans:
        ...
        filter_depth(scan_folder, out_folder, os.path.join(args.outdir, 'mvs{:0>3}_l3.ply'.format(scan_id)))
